...
BaseQL is a Go framework for rapidly building powerful graphql servers.
BaseQL has support for schemas automatically generated from Go types, live
queries, query batching, and more. BaseQL is an open-source project from
{basecode}. Its a fork of the original [thunder](https://github.com/samsarahq/thunder/).

# Feature Lightning Tour

BaseQL has a number of features to make it easy to build sophisticated
schemas. This section provides a brief overview of some of them.

## Reflection-based schema building

BaseQL generates resolvers automatically from Go struct types and function
...
BaseQL has support for automatically updating queries using _resolver
...
Using BaseQL's lightweight `sqlgen` and `livesql` ORM, it's easy to write
...
BaseQL automatically runs independent resolvers in different goroutines to
quickly compute complex queries. To keep large queries efficient, BaseQL has
support for built-in batching similar to Facebook's `dataloader`. With
batching, BaseQL automatically combines many parallel individual calls to a
`batch.Func`'s `Invoke` function into a single call to `Many` function.

Batching is very useful when fetching related objects from a SQL database. BaseQL's
...
BaseQL will execute `SELECT * FROM posts` and, if that returns three posts
with author IDs `10`, `20`, and `31`, a follow-up query `SELECT * FROM
authors WHERE id IN (10, 20, 31)`.

## Built-in graphiql

To get started quickly without wrangling any JavaScript, BaseQL comes with
...
keep packages reasonably-sized, BaseQL's schema builder supports extending a
...
> First, a fair warning. The BaseQL library is still a little bit tricky to use
> outside of {basecode}. The examples above and below work, but eg. the `npm` client
> still requires some wrangling.

## A minimal complete server

The program below is a fully-functional graphql server written using BaseQL. It
...
import (
  ...
  "github.com/base-go/baseql/graphql/schemabuilder"
...
func main() {
  ...
  server := &server{
    posts: []post{
      ...
      {Title: "graphql", Body: "did you hear about BaseQL?", CreatedAt: time.Now()},
    },
  }
...
## Using BaseQL without Websockets (POST requests)

For use with non-live clients (e.g. [Relay](https://facebook.github.io/relay/), [Apollo](https://www.apollographql.com/client/)) baseql provides an HTTP handler that can serve
POST requests, instead of having the client connect over a websocket. In this mode, baseql
...
BaseQL can emit a GraphQL introspection query schema useful for compatibility with
...
- The example/ directory contains a basic BaseQL application.
- The graphql/ directory contains BaseQL's graphql parser and executor.
- The reactive/ directory contains BaseQL's core dependency-tracking and
  live-update mechanism.
- The batch/ directory contains BaseQL's batching package.
- The diff/ and merge/ directories contain BaseQL's JSON diffing library
  used for live queries.
- The livesql/ directory contains a BaseQL driver for MySQL.
- The sqlgen/ directory contains a lightweight SQL query generator used by
  livesql/.

# Status

BaseQL has proven itself in production use at {basecode} for close to two
